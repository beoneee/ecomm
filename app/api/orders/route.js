...
export async function POST(request) {
  try {
    ...
    const{city,country,district,email,firstName,lastName,paymentMethod,phone,shippingCost,streetAddress,userId,}=checkoutFormData
    ...
    const result = await db.$transaction(async (prisma) => {
      ...
      const newOrder = await prisma.order.create({
        data:{city,country,district,email,firstName,lastName,paymentMethod,phone,shippingCost:parseFloat(shippingCost),streetAddress,userId,orderNumber: generateOrderNumber(8)}
      });
